...
<body style="margin:0; padding:0; background:#f5f5f5; font-family: Arial, sans-serif;">
    <div style="max-width:600px; margin:30px auto; background:#ffffff; border-radius:10px; overflow:hidden; border:1px solid #e5e5e5;">
        <div style="padding:20px; background:#dc2626; color:#ffffff;">
            <h2 style="margin:0;">Vendor Account Rejected </h2>
        </div>

        <div style="padding:20px; color:#111827;">
            <p style="font-size:16px;">Hello <b>{{ $vendorUser->name ?? 'there' }}</b>,</p>
            <p style="font-size:15px; line-height:1.6;">
                Your vendor request was rejected. Please review and resubmit your documents.
            </p>

            @if(!empty($reason))
                <div style="background:#fff7ed; padding:12px; border-radius:8px; font-size:14px; margin:15px 0;">
                    <b>Reason:</b> {{ $reason }}
                </div>
            @endif

            <p style="margin:25px 0;">
                <a href="{{ url('/vendor/documents') }}"
