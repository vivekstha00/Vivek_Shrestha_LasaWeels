...
<body style="margin:0; padding:0; background:#f5f5f5; font-family: Arial, sans-serif;">
    <div style="max-width:600px; margin:30px auto; background:#ffffff; border-radius:10px; overflow:hidden; border:1px solid #e5e5e5;">
        <div style="padding:20px; background:#7c3aed; color:#ffffff;">
        ...
        <div style="padding:20px; color:#111827;">
            ...
            <div style="background:#f9fafb; padding:12px; border-radius:8px; font-size:14px; line-height:1.8;">
                <b>Booking ID:</b> {{ $booking->id ?? ($payment->booking_id ?? '-') }} <br>
                <b>Amount:</b> Rs. {{ $payment->amount ?? '-' }} <br>
                <b>Reference:</b> {{ $payment->reference ?? ($payment->pidx ?? '-') }}
            </div>

            <p style="margin:25px 0;">
                <a href="{{ url('/user/bookings/' . ($booking->id ?? $payment->booking_id)) }}"
            ...
            <p style="font-size:14px; color:#374151;">
                Thank you for choosing <b>LasaWheels</b>.
            </p>
        </div>
